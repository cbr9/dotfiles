  ...
  home-manager.users.cabero.xdg.configFile."awesome/rc.lua" = {
    ...
    text = ( # lua
      ''
        ...
        local terminal = "${hm.programs.kitty.package}/bin/kitty"
        ...
        local function xprop()
          awful.spawn.easy_async("${pkgs.xorg.xprop}/bin/xprop", function(output, _, _, _)
        ...
        -- awful.key({ super, }, "s", hotkeys_popup.show_help),
          ...
          awful.key({ super }, "z", function() awful.spawn("${pkgs.zotero}/bin/zotero") end)
        )
